pipeline {
  ...
  stages {
    ...
    stage('Run Images') {
      steps {
        sh "docker run --restart unless-stopped --name server_img-${BUILD_TAG} --network host -v /home/jenkins/upload:/usr/src/app/server/upload -e ENV_PORT=${ENV_PORT} -e ENV_DATABASE=${ENV_DATABASE} -e ENV_HOST=${ENV_HOST} -d raptor2103/server:${BUILD_TAG}"
