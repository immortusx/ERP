...
router.get("/get-allVillage", tokenCheck, async (req, res) => {
  ...
  try {
    await db.query(
      `select vt.*,dt.name as DistrictName,st.state_name as stateName,tt.name as TalukaName from village vt 
      ...
      left join (select state_id,state_name from state) st on st.state_id = vt.state_id where vt.is_active = 1`,
      (err, allVillage) => {
        if (err) {
          console.log({ isSuccess: false, result: "error" });
          res.send({ isSuccess: false, result: "error" });
...
router.post("/add-Village", tokenCheck, async (req, res) => {
  ...
  const newUrl =
    "SELECT * FROM village where is_active = 1 and name ='" +
    capitalFirstLetter +
    "'";
...
router.get("/get-Villagebyid/:id", tokenCheck, async (req, res) => {
  ...
  try {
    ...
    left join (select id,name from taluka) tt on tt.id = vt.taluka_id
    left join  (select id,name from district) dt on dt.id = vt.district_id
    left join (select state_id,state_name from state) st on st.state_id = vt.state_id where vt.is_active = 1 and vt.id=` +
        VillageById,
      (err, VillagesIdData) => {
        if (err) {
          console.log({ isSuccess: false, result: "error" });
...
router.get("/get-Villagebydistrictid/:id", tokenCheck, async (req, res) => {
  ...
  try {
    ...
    await db.query(
      "SELECT * FROM Village where is_active = 1 and district_id=" +
        VillageByDistrictid,
      (err, VillageByDistrictidData) => {
        if (err) {
          console.log({ isSuccess: false, result: "error" });
          res.send({ isSuccess: false, result: "error" });
        } else {
          ...
          res
            .status(200)
            .send({ isSuccess: true, result: VillageByDistrictidData });
        }
      }
    );
  } catch (e) {
